...
class GAConfig:
    ...
    # Seed & pool
    seed_file: str = "XYZ1_original.npy"   # saved via: np.save("XY_original.npy", cp.asarray(seq_gpu))
...
def run_ga(cfg: GAConfig, outdir: Path, res: cr.GPUResources) -> None:
    ...
    initial_mol = cp.asarray(np.load("mol_post_XY.npy"))
    # DEAP setup
...
if __name__ == "__main__":
    ...
    outdir = Path("ga_runs")
    cfg = GAConfig(
        n_gen=40, pop_size=10, cx_prob=0.7, mut_prob=0.6,
