...
class OmegaTimeGAConfig:
    ...
    det_offset_sigma_init: float = 1000.0  # initial sigma for detuning offsets (Hz)
...
def score_sequence_from_scales(
    ...
    if genes.shape[0] != 3 * M:
        raise ValueError(f"genes must have length 3*M={3*M}, got {genes.shape[0]}")

    genes = np.asarray(genes, dtype=float)
    omega_scales = genes[:M]
    t_scales     = genes[M:2*M]
    det_offsets  = genes[2*M:3*M]   # Hz, additive per pulse type
    ...
    cr.raman_cool_with_pumping(molecules_dev=mol_SLM, pulses_dev=pulses_host, res=res,
                              K_max=int(cfg.K_max), show_progress=False)
    cr.raman_cool_with_pumping(molecules_dev=mol_AOD, pulses_dev=pulses_host, res=res,
                              K_max=int(cfg.K_max), show_progress=False)
...
def run_omega_time_ga(cfg: OmegaTimeGAConfig, res: cr.GPUResources) -> None:
    ...
    # Load base sequence (axis, delta_n, Omega_lin_Hz, t_sec, detuning_Hz)
    ...
    def init_ind() -> creator.Individual:
        omega_scales = clamp_array(
            rng.normal(1.0, cfg.omega_scale_sigma_init, size=M),
            cfg.omega_scale_min, cfg.omega_scale_max
        )
        t_scales = clamp_array(
            rng.normal(1.0, cfg.t_scale_sigma_init, size=M),
            cfg.t_scale_min, cfg.t_scale_max
        )
        det_offsets = clamp_array(
            rng.normal(0.0, cfg.det_offset_sigma_init, size=M),
            -cfg.pulse_freq_lim, cfg.pulse_freq_lim
        )
        vec = np.concatenate([omega_scales, t_scales, det_offsets])
    ...
    def evaluate(ind: np.ndarray) -> tuple:
        ...
        score_SLM, surv_SLM, nzbar_SLM,
        score_AOD, surv_AOD, nzbar_AOD,
        omega_vec, time_vec, det_vec) = score_sequence_from_scales(
            axes, dns, inverse,
    ...
    def mate(a: np.ndarray, b: np.ndarray):
        ...
        c1[:M]        = clamp_array(c1[:M],        cfg.omega_scale_min, cfg.omega_scale_max)
        c2[:M]        = clamp_array(c2[:M],        cfg.omega_scale_min, cfg.omega_scale_max)
        c1[M:2*M]     = clamp_array(c1[M:2*M],     cfg.t_scale_min,     cfg.t_scale_max)
        c2[M:2*M]     = clamp_array(c2[M:2*M],     cfg.t_scale_min,     cfg.t_scale_max)
        c1[2*M:3*M]   = clamp_array(c1[2*M:3*M],   -cfg.pulse_freq_lim, cfg.pulse_freq_lim)
        c2[2*M:3*M]   = clamp_array(c2[2*M:3*M],   -cfg.pulse_freq_lim, cfg.pulse_freq_lim)
    ...
    def mutate(ind: np.ndarray):
        # omega scales
        ind[:M] = clamp_array(
            ind[:M] + rng.normal(0.0, cfg.omega_scale_sigma_init, size=M),
            cfg.omega_scale_min, cfg.omega_scale_max
        )
        # t scales
        ind[M:2*M] = clamp_array(
            ind[M:2*M] + rng.normal(0.0, cfg.t_scale_sigma_init, size=M),
            cfg.t_scale_min, cfg.t_scale_max
        )
        # det offsets
        ind[2*M:3*M] = clamp_array(
            ind[2*M:3*M] + rng.normal(0.0, cfg.det_offset_sigma_init, size=M),
            -cfg.pulse_freq_lim, cfg.pulse_freq_lim
        )
        return (ind,)
    ...
    baseline_ind = creator.Individual(
        np.concatenate([np.ones(M), np.ones(M), np.zeros(M)]).astype(float)
    )
    pop[0] = baseline_ind
    ...
    for gen in range(cfg.n_gen):
        ...
        # Build & save corresponding full (P,5) sequence
        genes = np.asarray(best_ind, dtype=float)
        omega_scales = genes[:M]
        t_scales     = genes[M:2*M]
        det_offsets  = genes[2*M:3*M]
        ...
        meta = {
            ...
            "detuning_vector": det_lin_vec.tolist(),        # per type (before per-pulse clamp)
            "omega_scales": omega_scales.tolist(),
            "time_scales": t_scales.tolist(),
            "detuning_offsets": det_offsets.tolist(),       # NEW
        }
    ...
    for rank, ind in enumerate(hof, 1):
        genes = np.asarray(ind, dtype=float)
        omega_scales = genes[:M]
        t_scales     = genes[M:2*M]
        det_offsets  = genes[2*M:3*M]
        ...
        meta = {
            ...
            "omega_scales": omega_scales.tolist(),
            "time_scales": t_scales.tolist(),
            "detuning_offsets": det_offsets.tolist(),
        }
