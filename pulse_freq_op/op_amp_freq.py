...
class OmegaTimeGAConfig:
    ...
    # HARD CAP on Rabi frequency (linear Hz) per axis: (X, Y, Z)
    # Requirement: X/Y <= 30 kHz, Z <= 10 kHz
    omega_lin_max_xyz: Tuple[float, float, float] = (30e3, 30e3, 10e3)
...
def axis_caps_for_types(unique_pairs: np.ndarray, cfg: OmegaTimeGAConfig) -> np.ndarray:
    """
    unique_pairs: (M,2) with column 0 = axis in {0,1,2}
    Returns caps: (M,) max omega_lin (Hz) for each pulse type based on axis.
    """
    caps_xyz = np.asarray(cfg.omega_lin_max_xyz, dtype=float)
    if caps_xyz.shape != (3,):
        raise ValueError("cfg.omega_lin_max_xyz must be length-3 tuple (X,Y,Z)")

    axes_M = unique_pairs[:, 0].astype(int)
    if np.any((axes_M < 0) | (axes_M > 2)):
        raise ValueError("axis values must be in {0,1,2}")

    return caps_xyz[axes_M]
...
def score_sequence_from_scales(
    ...
    caps_xyz = np.asarray(cfg.omega_lin_max_xyz, dtype=float)  # (3,)
    if caps_xyz.shape != (3,):
        raise ValueError("cfg.omega_lin_max_xyz must be length-3 tuple (X,Y,Z)")

    # Cap per pulse-type (M) by that type's axis
    omega_caps_M = caps_xyz[unique_pairs[:, 0].astype(int)]  # (M,)

    # Cap per pulse (P) by that pulse's axis
    axes_P = axes.astype(int)
    omega_caps_P = caps_xyz[axes_P]  # (P,)
    ...
    omega_lin_vec = np.minimum(np.maximum(omega_lin_vec, 0.0), omega_caps_M)  # <-- per-type axis cap
    ...
    omega_lin_full = np.minimum(np.maximum(omega_lin_full, 0.0), omega_caps_P)  # <-- per-pulse axis cap
...
def run_omega_time_ga(cfg: OmegaTimeGAConfig, res: cr.GPUResources) -> None:
    ...
    # HARD CAP any baseline Ω coming from the file, per axis (X/Y/Z)
    omega_caps_M = axis_caps_for_types(unique_pairs, cfg)
    baseline_omega_lin = np.minimum(np.maximum(baseline_omega_lin, 0.0), omega_caps_M)
    ...
    # Per-Z-type maximum omega scale allowed by Z hard Ω cap:
    # baseline_omega_lin[z] * omega_scale_z <= omega_lin_max_xyz[2]
    cap_z = float(cfg.omega_lin_max_xyz[2])
    eps = 1e-12
    omega_scale_cap_z = cap_z / (baseline_omega_lin[z_idx] + eps)
    omega_scale_cap_z = np.clip(omega_scale_cap_z, 0.0, np.inf)
    ...
    def init_ind() -> creator.Individual:
        omega_scales_z = rng.normal(1.0, cfg.omega_scale_sigma_init, size=Mz)
        omega_scales_z = clamp_array(omega_scales_z, cfg.omega_scale_min, cfg.omega_scale_max)
        omega_scales_z = np.minimum(omega_scales_z, omega_scale_cap_z)
    ...
    def mate(a: np.ndarray, b: np.ndarray):
        ...
        # Enforce Z per-type omega scale cap implied by Z omega_lin_max
        c1[:Mz]        = np.minimum(c1[:Mz], omega_scale_cap_z)
        c2[:Mz]        = np.minimum(c2[:Mz], omega_scale_cap_z)
    ...
    def mutate(ind: np.ndarray):
        ...
        ind[:Mz] = np.minimum(ind[:Mz], omega_scale_cap_z)
    ...
    # Baseline individual (Z only): scales=1, offsets=0 (respect Z omega cap)
    baseline_omega_scales_z = np.minimum(np.ones(Mz), omega_scale_cap_z)
    baseline_ind = creator.Individual(
        np.concatenate([baseline_omega_scales_z, np.ones(Mz), np.zeros(Mz)]).astype(float)
    )
    pop[0] = baseline_ind
    ...
    for gen in range(cfg.n_gen):
        ...
        omega_lin_vec = np.minimum(np.maximum(omega_lin_vec, 0.0), omega_caps_M)  # HARD CAP per axis/type
        t_sec_vec     = baseline_t_sec * t_scales_full
        det_lin_vec   = baseline_det_lin + det_offsets_full

        omega_lin_full = omega_lin_vec[inverse]
        caps_pulse = np.asarray(cfg.omega_lin_max_xyz, dtype=float)[axes.astype(int)]
        omega_lin_full = np.minimum(np.maximum(omega_lin_full, 0.0), caps_pulse)  # HARD CAP per axis/pulse
        t_sec_full     = t_sec_vec[inverse]
        det_full       = clamp_array(det_lin_vec[inverse], -float(cfg.pulse_freq_lim), float(cfg.pulse_freq_lim))
        ...
        meta = {
            ...
            "omega_vector": omega_lin_vec.tolist(),   # per-type Ω after axis caps
            ...
            "omega_lin_max_xyz": list(map(float, cfg.omega_lin_max_xyz)),
        }
...
if __name__ == "__main__":
    ...
    cfg = OmegaTimeGAConfig(
        ...
        K_max=30,
        base_seq_npy="seq_cap.npy",
        outdir="ga_runs",
        omega_scale_min=0.001,
        omega_scale_max=100.0,
        t_scale_min=0.001,
        t_scale_max=100.0,
        pulse_freq_lim=20e3,
        omega_lin_max_xyz=(20e3, 20e3, 10e3),  # X,Y,Z caps
        trap_shift=-2e3,
        carrier_shift=3e3,
